...
;;
;; TODO's
;;   [x] program flags/options parsing
;;   [x] markdown generation
;;       [x] post .html generation additions
;;   [x] index generation
;;   [x] config file parsing (what should we be configging???)
;;   [] rsync call
;;   [] rss/atom feed
...
(define gen-mode (make-parameter #f))
(define conf-mode (make-parameter #f))

(define rackle
  (command-line
   #:program "rackle"
   #:once-any
   [("-p" "--post") "Post to website via rsync, will generate new website before posting" (post-mode #t)]
   [("-g" "--generate") "generate new website" (gen-mode #t)]
   #:once-each
   [("-c" "--config-path") "set config path (default is config.toml)" (conf-mode #t)]
   #:args (file-path)
   file-path))
...
(define (convert-posts-to-path dest-path post)
  (let ((out-file (open-output-file (string-append dest-path "/" (to-html (get-post-name post))) #:exists 'replace #:mode'text))
...
(define (get-post-list in-path)
  (let* ((posts-path (string-append-paths in-path "/_site/posts"))
		 ...
		 (dir-path-list (map (curry string-append "/_site/posts/") (map path->string dir-list))))
...
(define (create-site in-path)
  (create-posts   (string->path in-path)) ;; todo
  (create-about   (string->path in-path))
  (create-index   (string->path in-path)) ;; todo
