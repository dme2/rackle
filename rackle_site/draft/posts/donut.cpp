...
To recap, the usual use cases are asynchronous IO (networks, files, etc.) and generator functions (audio, images, etc.), which we'll be taking a look at soon.
...
/*
The following code calculates the donuts current frame. It's lifted directly from [here] (), with a nice explanation foun [here]() */
